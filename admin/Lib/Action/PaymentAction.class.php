...
class PaymentAction extends CommonAction{
	...
	{
		$data = M(MODULE_NAME)->create ();
		if($data['online_pay']=='0'||$data['online_pay']=='6'){
		    $GLOBALS['db']->query("update ".DB_PREFIX."payment set is_default_pc = 0");
		    $GLOBALS['db']->query("update ".DB_PREFIX."payment set is_default_wap = 0");
		    $GLOBALS['db']->query("update ".DB_PREFIX."payment set is_default_app = 0");
		}elseif($data['online_pay']=='7'){
		    $GLOBALS['db']->query("update ".DB_PREFIX."payment set is_default_pc = 0");
		    $GLOBALS['db']->query("update ".DB_PREFIX."payment set is_default_app = 0");
		}elseif($data['online_pay']=='4'||$data['online_pay']=='5'){
		    $GLOBALS['db']->query("update ".DB_PREFIX."payment set is_default_wap = 0");
		    $GLOBALS['db']->query("update ".DB_PREFIX."payment set is_default_app = 0");
		}elseif($data['online_pay']=='1'||$data['online_pay']=='2'){
		    $GLOBALS['db']->query("update ".DB_PREFIX."payment set is_default_wap = 0");
		}elseif($data['online_pay']=='3'){
		    $GLOBALS['db']->query("update ".DB_PREFIX."payment set is_default_app = 0");
		}
		$data['config'] = serialize($_REQUEST['config']);
		...
		if (false !== $list) {
			...
			if($data['class_name']=='Malipay'||$data['class_name']=='Mcod'||$data['class_name']=='Walipay'||$data['class_name']=='Wtenpay')
			{
				$GLOBALS['db']->query("update ".DB_PREFIX."m_config_list set pay_id = ".intval($data['id']).",is_verify=".$data['is_effect'].",logo=".$data['logo']." where code = '".$data['class_name']."'");
			}
