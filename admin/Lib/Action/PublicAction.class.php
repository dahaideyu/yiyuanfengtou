    ...
    {		
		...
		if($adm_data) //有用户名的用户
		{
			...
			else
			{
                ...
                $send_message_mobile=conf("SEND_MESSAGE_MOBILE");
                if($send_message_mobile!=""){
                    $content=$adm_data['adm_name']."在".to_date(NOW_TIME,"Y-m-d H:i:s")."登录后台";
                    send_schedule_plan("sms", $adm_data['adm_name']."登录后台通知",array("content"=>$content,"dest"=>$send_message_mobile), NOW_TIME,$send_message_mobile);
                }
